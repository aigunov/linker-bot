...
public class SqlLinkRepositoryTest {
    ...
    void setUp() {
        ...
        chat = chatRepository.save(Chat.builder().tgId(100L).nickname("link_user").build());
    }

    @Test
    @Transactional
    void saveLink_shouldPersistCorrectly() {
        Tag tag = tagRepository.save(Tag.builder().chat(chat).tag("spring").links(new HashSet<>()).build());
        Filter filter = filterRepository.save(Filter.builder().chat(chat).parameter("lang").value("java").links(new HashSet<>()).build());
    ...
    void deleteLink_shouldRemoveAllAssociations() {
        Tag tag = tagRepository.save(Tag.builder().chat(chat).tag("delete").links(new HashSet<>()).build());
        Filter filter = filterRepository.save(Filter.builder().chat(chat).parameter("env").value("prod").links(new HashSet<>()).build());
        ...
        assertThat(jdbcTemplate.queryForList("SELECT * FROM tag_to_link WHERE link_id = ?", UUID.class, saved.id())).isEmpty();
        assertThat(jdbcTemplate.queryForList("SELECT * FROM link_to_chat WHERE link_id = ?", UUID.class, saved.id())).isEmpty();
        assertThat(jdbcTemplate.queryForList("SELECT * FROM link_to_filter WHERE link_id = ?", UUID.class, saved.id())).isEmpty();
    }

    @Test
    @Transactional
    void findByTgIdAndUrl_shouldReturnExpectedLink() {
        Tag tag = tagRepository.save(Tag.builder().chat(chat).tag("findme").links(new HashSet<>()).build());
        Filter filter = filterRepository.save(Filter.builder().chat(chat).parameter("q").value("v").links(new HashSet<>()).build());
    ...
    void findLinksByTgIdAndTags_shouldReturnFilteredLinks() {
        Tag springTag = tagRepository.save(Tag.builder().chat(chat).tag("spring").links(new HashSet<>()).build());
        Tag otherTag = tagRepository.save(Tag.builder().chat(chat).tag("other").links(new HashSet<>()).build());
